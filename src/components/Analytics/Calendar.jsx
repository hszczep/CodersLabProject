...
import moment from 'moment';
// import { useHistory } from 'react-router-dom';
import './ScssCalendar/Calendar.css';
...
export default function Calendars({events}) {
  ...
  //     history.push({
  //       pathname: '/calendar',
  //       search: `?day=${date.getDate()}`
  //     });
  //   }
  // };
  
  return (
    <div className="app" id="color-calendar">
      <Calendar
        events={events}
        value={selectedDate}
        //onClickDay={onClickDay}
        tileContent={({ date, view }) => {
          if (events.find( e => moment(e.startDate).format("DD-MM-YYYY") === moment(date).format("DD-MM-YYYY"))){
            return ( 
              <div   
                style={{ padding:0, margin:0, height:0.5, display:"block", textDecoration:"none" }}
              >&bull;</div>
            );
          };
        }}
